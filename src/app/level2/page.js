...
System: The response was cut off at the table in Lesson 4 due to exceeding the maximum length. Below is the completed `Level2` component, continuing from where the previous response ended, ensuring all requested topics (`if`, `elif`, `else`, `case`, `for`, `while`, `until`, `break`, `continue`, and small automation scripts) are covered with consistent styling, validation, and logic as in `Level1` (Bash Basics) and the Python levels (6–9). The artifact maintains the same structure, including `"use client"`, `useState`, `useEffect`, subscription and `level1Completed` checks, inline styles for `<pre>` and textarea, Tailwind classes, and regex-based validation.

<xaiArtifact artifact_id="dc69c971-9e9e-4cf1-89a8-e5a7e1f6f571" artifact_version_id="4f592492-558d-424a-94f7-a9d43ef8ce73" title="page.js" contentType="text/javascript">"use client";
import { useState, useEffect } from "react";
import Link from "next/link";
...
export default function Level2() {
  ...
  const lessons = [
    ...
    },
    {
          ...
          <p className="mt-2">
            ...
            <table className="border border-green-400 border-collapse w-full mt-2">
              ...
              <tbody>
                ...
                <tr className="border border-green-400">
                  <td className="border border-green-400 p-2">for/while</td>
      ...
      check: (code) => {
        const result = /((if\s+\[.*\]\s*;\s*then|case\s+.*\s+in)(.*(for\s+\w+\s+in|while\s+\[.*\]\s*;\s*do))/ms.test(code);
        console.log(`Lesson 4 Check: Code="${code}", Result=${result}`);
        return result;
      },
      error: '❌ Try again. Combine conditionals (if/case) with loops (for/while) in your script.',
